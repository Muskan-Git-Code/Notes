...
children: Map<Character, TrieNode>, isEnd: boolean, freq: int

TypeaheadService

insert(word), getSuggestions(prefix), updateFrequency(word)
...
# System Design Summaries (Markdown Format)

---

## 1. Design Instagram / Twitter / Facebook
...
UI → FeedService / PostService / NotificationService ↔ Cache ↔ DB
...
| Like         | likeId, userId, postId                              |
| FeedService  | getFeed(userId), updateFeed(userId, post)           |
| Notification | type, timestamp, senderId, receiverId               |
...
UI → BookingService / MovieService / PaymentService ↔ DB
...
UI → OrderService / RestaurantService / DeliveryService ↔ DB
...
| Message | messageId, senderId, receiverId, text, timestamp  |
| Group   | groupId, name, members\[]                         |
| Chat    | chatId, messages\[], participants\[], lastMessage |
...
* Generate a shorter url, short url redirect to actual url page, link expiry date

### Flow Diagram

UI → Request(Take Long URL) → Give shorten URL ↔ Database (through cache)

### Class Structure Table

| Entity     | Attributes                                                                          |
| ---------- | ----------------------------------------------------------------------------------- |
| User       | id, name, email, password, create\_date                                             |
| URL\_Table | id, original\_url, shorten\_url, create\_date, expiry\_date, user\_id, click\_count |
